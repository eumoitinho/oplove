...
export function MessagesView() {
  ...
  useEffect(() => {
    if (user) {
      ...
      if (conversations.length > 0 && !isConversationsStale()) {
        console.log('[MessagesView] Using cached conversations')
        return
      }
      loadConversations()
    } else {
  ...
  const loadConversations = async () => {
    try {
      ...
      console.log('[MessagesView] Loading conversations for user:', user!.id)
      const data = await messagesService.getConversations(user!.id)
      setConversations(data)
    } catch (error) {
      console.error('[MessagesView] Error loading conversations:', error)
  ...
  return (
    <div className="fixed inset-0 top-[64px] bottom-[80px] lg:fixed lg:inset-0 lg:top-[80px] lg:bottom-0 w-full flex bg-white dark:bg-black overflow-hidden">
      {/* Conversations List - Narrow like Twitter/X */}
      <div className={`${selectedConversationId ? 'hidden lg:flex' : 'flex'} w-full lg:w-[380px] flex-col border-r border-gray-200 dark:border-white/10 bg-white dark:bg-black`}>
        {/* Header Fixo */}
        <div className="sticky top-0 z-20 p-4 border-b border-gray-200 dark:border-white/10 bg-white dark:bg-black">
      ...
      {/* Chat Area - Takes remaining space like Twitter/X */}
      {selectedConversationId ? (
        <div className="flex-1 flex flex-col h-full bg-white dark:bg-black">
          {/* Chat Header - Fixo no Topo */}
          <div className="sticky top-0 z-20 p-3 sm:p-4 border-b border-gray-200 dark:border-white/10 bg-white dark:bg-black flex items-center justify-between">
          ...
          <div className="sticky bottom-0 z-20 p-3 sm:p-4 border-t border-gray-200 dark:border-white/10 bg-white dark:bg-black">
